...
RealNposSensor::RealNposSensor()
{
    ...
    // //bynav内置惯导比例因子 x1-3H 我们自己的设备
    // gro_scalefactor = 3.35276126861572e-05 * (M_PI / 180);
    // acc_scalefactor = 4.65661287307739e-06;

    //bynav内置惯导比例因子 x1-5H 我们自己的设备
    gro_scalefactor = 3.0517578125e-05 * (M_PI / 180);
    acc_scalefactor = 3.74094009399414e-06;

    //bynav内置惯导比例因子 x1-6H 陶贤露的设备
    // gro_scalefactor = 2.88991928100586e-05 * (M_PI / 180);
    // acc_scalefactor = 7.48188018798828e-06;
    //NPOS320内置惯导比例因子
...
bool RealNposSensor::open()
{
    ...
    // //    // Set the baud rate of the serial port. 复杂的北云
    serial_port.SetBaudRate(BaudRate::BAUD_921600); //set the baudrate of serial port
    
    // Set the baud rate of the serial port.简单的北云
    // serial_port.SetBaudRate(BaudRate::BAUD_921600); //set the baudrate of serial port
